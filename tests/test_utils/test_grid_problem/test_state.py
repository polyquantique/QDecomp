...
from cliffordplust.grid_problem.grid_operator import *
from cliffordplust.grid_problem.state import *
...
def test_invalid_initialization():
    with pytest.raises(ValueError):
        State(np.array([[mp.mpf(1), mp.mpf(2)], [mp.mpf(2), mp.mpf(1)]]), np.array([[mp.mpf(1), mp.mpf(2)], [mp.mpf(2), mp.mpf(1)]]))
    with pytest.raises(ValueError):
