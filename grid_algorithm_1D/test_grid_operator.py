...
valid_entries = [
    1, 2, 5, 
    D(1, 1), D(3, 5), D(2, 3), 
    Zsqrt2(1, 1), Zsqrt2(10, 8), Zsqrt2(4, 4), 
    Dsqrt2(D(1, 1), D(1, 3)), Dsqrt2(D(2, 2), D(1, 0))
]
...
@pytest.mark.parametrize("grid_op_list", [
    [np.random.choice(valid_entries), np.random.choice(valid_entries), np.random.choice(valid_entries), np.random.choice(valid_entries)] for _ in range(20)
])
...
def test_grid_operator_list_error(invalid_list):
    with pytest.raises(ValueError, match="G must be a 4-element flat list or a 2x2 nested list with valid elements."):
...
# Parametrize the test to run 20 times
@pytest.mark.parametrize("grid_op_array", [
    np.array([[np.random.choice(valid_entries), np.random.choice(valid_entries)], [np.random.choice(valid_entries), np.random.choice(valid_entries)]], dtype=object) for _ in range(20)
])
...
# Parametrize the test to run 20 times
@pytest.mark.parametrize("grid_op_array", [
    np.array([[np.random.choice(invalid_entries), np.random.choice(valid_entries)], [np.random.choice(valid_entries), np.random.choice(valid_entries)]], dtype=object) for _ in range(20)
])
...
@pytest.mark.parametrize("grid_op, expected_det", [(I, 1), (R, 1), (K, 1), (X, -1), (Z, -1), (A, 1), (B, 1)])
...
def test_inv_invalid(grid_op):
    ...
    with pytest.raises(ValueError, match="The inversion is not defined|Determinant must be non-zero"):
        grid_op.inv()
...
add_sub_expected = [
    ...
    Grid_Operator([
        Dsqrt2(D(1, 0), D(1, 1)), Dsqrt2(D(0, 0), D(1, 1)), 
        Dsqrt2(D(0, 0), D(1, 1)), Dsqrt2(D(1, 0), D(1, 1))
    ]),  # R + B
    Grid_Operator([
        Dsqrt2(D(-1, 0), D(1, 1)), Dsqrt2(D(-1, 0), D(-1, 1)), 
        Dsqrt2(D(0, 0), D(1, 1)), Dsqrt2(D(0, 0), D(1, 1))
    ]),  # K - X
...
@pytest.mark.parametrize("scalar, grid_op", [(np.random.choice(valid_entries), np.random.choice(grid_ops)) for _ in range(20)])
