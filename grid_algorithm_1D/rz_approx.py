...
from Steiner_ellipse.steiner_ellipse import steiner_ellipse_def, ellipse_bbox, is_inside_ellipse, plot_ellipse
...
U = np.array(z_rotational_approximation(epsilon, theta), dtype=complex)
rz = np.array([
    [np.exp(-theta / 2 * 1j), 0], 
    [0, np.exp(theta / 2 * 1j)]
])
E = np.linalg.norm(rz - U, 2)
print(E/epsilon)