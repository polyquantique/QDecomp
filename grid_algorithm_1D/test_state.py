...
def test_reduce_zero_determinant_raises_error():
    ...
    with pytest.raises(ValueError, match="The determinant of A and B must be positive and non-zero"):
        State(A, B)
...
def test_shift_positive():
    ...
    exp_A = np.array([[float(lamb ** 2), math.sqrt(2)], [math.sqrt(2), 3 * float(inv_lamb ** 2)]], dtype=float)
    exp_B = np.array([[5 * float(inv_lamb ** 2) / math.sqrt(2), -3], [-3, 2 * float(lamb ** 2) * math.sqrt(2)]], dtype=float)
...
def test_shift_negative():
    A = np.array([[1, math.sqrt(2)], [math.sqrt(2), 3]], dtype=float)
    B = np.array([[5 / math.sqrt(2), -3], [-3, 2 * math.sqrt(2)]], dtype=float)
    exp_A = np.array([[float(inv_lamb), math.sqrt(2)], [math.sqrt(2), 3 * float(lamb)]], dtype=float)
    exp_B = np.array([[5 * float(lamb) / math.sqrt(2), 3], [3, 2 * float(inv_lamb) * math.sqrt(2)]], dtype=float)
