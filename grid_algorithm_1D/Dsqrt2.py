...
class D:
    ...
    def __mul__(self, other: D | int) -> D:
        ...
        if not isinstance(other, (D, int, np.int32, np.int64)):
            raise TypeError(
                f"Product operation is not defined for the {type(other).__name__} class."
            )
        if isinstance(other, D):
...
class Dsqrt2:
    ...
    def __init__(self, a: tuple[int, int] | D, b: tuple[int, int] | D) -> None:
        ...
        if not (
            isinstance(a, int)
            or isinstance(a, D)
            or (isinstance(a, tuple) and len(a) == 2 and all(isinstance(i, int) for i in a))
        ):
            raise TypeError(
                "Argument `a` must be an instance of `D` or a tuple of two integers (int, int)."
            )
        ...
        if not (
            isinstance(b, int)
            or isinstance(b, D)
            or (isinstance(b, tuple) and len(b) == 2 and all(isinstance(i, int) for i in b))
        ):
            raise TypeError(
                "Argument `b` must be an instance of `D` or a tuple of two integers (int, int)."
            )
