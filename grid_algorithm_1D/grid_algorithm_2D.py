...
def solve_grid_problem_2D(
    A: Sequence[Sequence[float | int]], B: Sequence[Sequence[float | int]]
) -> list[Zomega]:
    """
...
def plot_solutions(
    ...
    plt.scatter(
        alpha_conjugate_x, alpha_conjugate_y, color="red", marker="x", label="$\\alpha^\\bullet$"
    )
    ...
    plt.ylabel("Im")
    plt.title(
        f"2D grid problem for A = {list(A[0])} x {list(A[1])} and B = {list(B[0])} x {list(B[1])}"
    )
