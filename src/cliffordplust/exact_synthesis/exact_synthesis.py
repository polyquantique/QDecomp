...
"""
This module allows to do the exact synthesis of any 2x2 unitary with entries in Doemga into
a sequence of H and T gates up to a global phase. This module also contains multiple 
functions to help characterize and generate sequences of H and T gates. 

Algorithm is based on the paper by Vadym Kliuchnikov, Dmitri Maslov, and Michele Mosca : 
Fast and efficient exact synthesis of single qubit unitaries generated by Clifford and T gates,
and can be found at https://arxiv.org/abs/1206.5236
"""
...
def exact_synthesis_alg(U: np.array) -> str:
    """Exact synthesis algorithm of a unitary 2x2 matrix with \u2145[\u03C9] elements into a sequence
    of W, H and T gates.
...
def exact_synthesis_reduc(U: np.array) -> tuple[str, np.array]:
    """Exact synthesis reduction of a unitary 2x2 matrix with \u2145[\u03C9] elements into a matrix with sde <3.

    Implementation of Algorithm 1 from paper by Vadym Kliuchnikov,
    ...
    while s > 3:
        # Look for k for which sde is reduced by 1
        for k in [0, 1, 2, 3]:
...
def lookup_sequence(U: np.array) -> str:
    """Lookup the sequence of W, H and T gates to synthesize a matrix with \u2145[\u03C9] elements
    Args:
    ...
    # Load s3_table into a dictionnary
    with open(
        os.path.join(os.path.dirname(__file__), "s3_table.json"), "r"
    ) as f:
        s3_dict = json.load(f)
        s3_dict = {
    ...
    for i in range(8):
        ...
        for key, value in s3_dict.items():
            ...
            if convert_to_tuple(U_t) == value:
                ...
                k = evaluate_omega_exponent(
                    U[1, 1], U[0, 0].complex_conjugate()
                )
                k_pp = evaluate_omega_exponent(
                    U_w[1, 1], U_w[0, 0].complex_conjugate()
                )
                k_prime = (k - k_pp) % 8
                key += "T" * k_prime
                key += "W" * ((8 - i) % 8)
...
def convert_to_tuple(array: np.array) -> tuple:
    ...
    """
    if not np.all(
        [isinstance(element, Domega) for element in array.flatten()]
    ):
        raise TypeError("Matrix elements must be of class Domega")
