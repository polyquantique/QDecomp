...
def exact_synthesis_alg(U: np.array, print_global_phase: bool = False) -> str:
    ...
    s3_sequence = s3_decomposition(u3, print_global_phase=print_global_phase)
...
def s3_decomposition(U: np.array, print_global_phase: bool = False) -> str:
    ...
    for i in range(8):
        ...
        for key, value in s3_dict.items():
            ...
            if convert_to_tuple(U_t) == value:
                ...
                # Compute k': real T exponnent
                k_prime = (k - k_pp) % 8
                key += "T" * k_prime
                if print_global_phase:
                    key += "W" * ((8 - i) % 8)
                return key
